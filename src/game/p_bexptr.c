...
void A_Spawn(mobj_t *mo)
{
    if (mo->state->misc1)
    {
        /*    mobj_t *newmobj = */ P_SpawnMobj(
            mo->x, mo->y, (mo->state->misc2 << FRACBITS) + mo->z, mo->state->misc1 - 1);
...
// [crispy] this is pretty much the only action pointer that makes sense for both mobj and pspr
// states
        ...
        //    if (autoaim || !beta_emulation)
        {
            ...
            if (critical->freeaim == FREEAIM_DIRECT)
                ...
                do
                {
                    ...
                    if (!linetarget)
                        slope = (critical->freeaim == FREEAIM_BOTH) ? PLAYER_SLOPE(player) : 0,
                        an = mo->angle;

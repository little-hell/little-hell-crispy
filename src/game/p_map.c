...
boolean PIT_CheckThing(mobj_t *thing)
{
    ...
    if (tmthing->flags & MF_MISSILE)
    {
        ...
        const fixed_t thingheight =
            (tmthing->target && tmthing->target->player && critical->freeaim == FREEAIM_DIRECT)
                                ...
                                (tmthing->target->type == MT_BRUISER && thing->type == MT_KNIGHT)))
        {
            ...
            // sdh: Add species_infighting here.  We can override the
...
void P_LineAttack(mobj_t *t1, angle_t angle, fixed_t distance, fixed_t slope, int damage)
{
    ...
    P_PathTraverse(t1x, t1y, x2, y2, PT_ADDLINES | PT_ADDTHINGS, PTR_ShootTraverse);
}

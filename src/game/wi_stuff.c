...
void WI_drawStats(void)
{
    ...
    if (sp_state == 10 &&
        ((gamemode != commercial && gamemap == 8) || (gameversion == exe_chex && gamemap == 5)))
    {
        acceleratestage = 1;
    }
...
void WI_Ticker(void)
{
    ...
    if (bcnt == 1)
    {
        ...
        // [crispy] Sigil
        else if (crispy->haved1e5 && wbs->epsd == 4 && W_CheckNumForName("D_SIGINT") != -1)
...
static void WI_loadUnloadData(load_callback_t callback)
{
    ...
    if (gamemode == commercial)
    {
        for (i = 0; i < NUMCMAPS; i++)
        {
            ...
            if (crispy->havemaster && crispy->havemaster != (char *) -1 && wbs->epsd == 2 &&
                i < 21) // [crispy] gamemission == pack_master
            {
                name[0] = 'M';
            }
    ...
    if (gamemode == commercial)
    {
        if (crispy->havenerve && wbs->epsd == 1 &&
            W_CheckNumForName("NERVEINT") != -1) // [crispy] gamemission == pack_nerve
        {
            M_StringCopy(name, "NERVEINT", sizeof(name));
        }
        else
        {
    ...
    else if (crispy->haved1e5 && wbs->epsd == 4 && W_CheckNumForName("SIGILINT") != -1) // [crispy]
                                                                                        // Sigil
    {
        M_StringCopy(name, "SIGILINT", sizeof(name));
    }
    else
    {

...
void P_LoadLineDefs(int lump)
{
    ...
    if (warn)
    {
        fprintf(
            stderr,
            "P_LoadLineDefs: Found %d line%s with unknown linedef type.\n",
            warn,
            (warn > 1) ? "s" : "");
    }
    // [crispy] warn about special linedefs without tag
    if (warn2)
    {
        fprintf(
            stderr,
            "P_LoadLineDefs: Found %d special linedef%s without tag.\n",
            warn2,
            (warn2 > 1) ? "s" : "");
    }
    if (warn || warn2)
...
boolean P_LoadBlockMap(int lump)
{
    ...
    if (M_CheckParm("-blockmap") || lump >= numlumps || (lumplen = W_LumpLength(lump)) < 8 ||
            ...
            do
            {
                ...
                if (!v->moved)
                {
                    ...
                    if (v != l->v1 && v != l->v2)
                    {
                        ...
                        v->r_x =
                            (fixed_t)((dx2 * v->x + dy2 * l->v1->x + dxy * (v->y - l->v1->y)) / s);
                        v->r_y =
                            (fixed_t)((dy2 * v->y + dx2 * l->v1->y + dxy * (v->x - l->v1->x)) / s);
                ...
                // [crispy] if v doesn't point to the second vertex of the seg already, point it
                // there
...
const char *skilltable[] = {"Nothing", "Baby", "Easy", "Normal", "Hard", "Nightmare"};
...
void P_SetupLevel(int episode, int map, int playermask, skill_t skill)
{
    ...
    if (crispy->havenerve || crispy->havemaster)
    {
        ...
        else if (crispy->havenerve && episode == 2)
        {
    ...
    // [crispy] better logging
    {
        ...
        fprintf(
            stderr,
            "P_SetupLevel: %s (%s) %s%s %d:%02d:%02d/%d:%02d:%02d ",
            maplumpinfo->name,
            W_WadNameForLump(maplumpinfo),
            skilltable[BETWEEN(0, 5, (int) skill + 1)],
            rfn_str,
            ltime / 3600,
            (ltime % 3600) / 60,
            ltime % 60,
    ...
    crispy_validblockmap =
        P_LoadBlockMap(lumpnum + ML_BLOCKMAP); // [crispy] (re-)create BLOCKMAP if necessary

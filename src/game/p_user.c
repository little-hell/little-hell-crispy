...
void P_MovePlayer(player_t *player)
{
    ...
    {
        player->lookdir = BETWEEN(
            -LOOKDIRMIN * MLOOKUNIT, LOOKDIRMAX * MLOOKUNIT, player->lookdir + cmd->lookdir);
    }
}
...
void P_PlayerThink(player_t *player)
{
    ...
    if (cmd->arti)
    {
        if ((cmd->arti & AFLAG_JUMP) && onground &&
            player->viewz < player->mo->ceilingz - 16 * FRACUNIT && !player->jumpTics)
        {
    ...
    if (cmd->buttons & BT_CHANGE)
    {
        ...
        if ((crispy->havessg) && newweapon == wp_shotgun && player->weaponowned[wp_supershotgun] &&

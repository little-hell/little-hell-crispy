...
void R_DrawSpanSolid(void)
{
    ...
    if (ds_x2 < ds_x1 || ds_x1 < 0 || ds_x2 >= SCREENWIDTH || (unsigned) ds_y > SCREENHEIGHT)
    {
        I_Error("R_DrawSpanSolid: %i to %i at %i", ds_x1, ds_x2, ds_y);
    }
#endif
...
void R_DrawSpanSolidLow(void)
{
    ...
    if (ds_x2 < ds_x1 || ds_x1 < 0 || ds_x2 >= SCREENWIDTH || (unsigned) ds_y > SCREENHEIGHT)
    {
        I_Error("R_DrawSpanSolidLow: %i to %i at %i", ds_x1, ds_x2, ds_y);
    }
#endif
...
void R_FillBackScreen(void)
{
    ...
    if (background_buffer == NULL)
    {
        background_buffer = Z_Malloc(
            MAXWIDTH * (MAXHEIGHT - SBARHEIGHT) * sizeof(*background_buffer), PU_STATIC, NULL);
    }
    ...
    for (x = 0; x < (scaledviewwidth >> crispy->hires); x += 8)
        V_DrawPatch(
            (viewwindowx >> crispy->hires) + x - WIDESCREENDELTA,
    ...
    for (y = 0; y < (viewheight >> crispy->hires); y += 8)
        V_DrawPatch(
            (viewwindowx >> crispy->hires) - 8 - WIDESCREENDELTA,
            (viewwindowy >> crispy->hires) + y,
            patch);
    patch = W_CacheLumpName("brdr_r", PU_CACHE);

    for (y = 0; y < (viewheight >> crispy->hires); y += 8)
        V_DrawPatch(
            (viewwindowx >> crispy->hires) + (scaledviewwidth >> crispy->hires) - WIDESCREENDELTA,
            (viewwindowy >> crispy->hires) + y,
            patch);

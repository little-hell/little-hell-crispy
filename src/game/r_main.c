...
angle_t
R_PointToAngleSlope(fixed_t x, fixed_t y, int (*slope_div)(unsigned int num, unsigned int den))
{
    x -= viewx;
    y -= viewy;
...
void R_ExecuteSetViewSize(void)
{
    ...
    else
    {
        ...
        if (crispy->widescreen)
        {
            ...
            scaledviewwidth =
                viewheight * SCREENWIDTH / (SCREENHEIGHT - (ST_HEIGHT << crispy->hires));
            // [crispy] make sure scaledviewwidth is an integer multiple of the bezel patch width
            scaledviewwidth =
                (scaledviewwidth + widescreen_edge_aligner) & (int) ~widescreen_edge_aligner;
...
void R_SetupFrame(player_t *player)
{
        ...
        leveltime > oldleveltime)
    {
        ...
        pitch = (player->oldlookdir +
                 (player->lookdir - player->oldlookdir) * FIXED2DOUBLE(fractionaltic)) /
                    MLOOKUNIT +
                (player->oldrecoilpitch +
                 FixedMul(player->recoilpitch - player->oldrecoilpitch, fractionaltic));
    }
    else
    {
        viewx = player->mo->x;
    ...
    tempCentery = viewheight / 2 +
                  (pitch * (1 << crispy->hires)) * (screenblocks < 11 ? screenblocks : 11) / 10;

...
boolean P_SetMobjState(mobj_t *mobj, statenum_t state)
{
    ...
    do
    {
        ...
        if (st->action.acp3)
            st->action.acp3(
                mobj, NULL, NULL); // [crispy] let pspr action pointers get called from mobj states
...
static void P_ExplodeMissileSafe(mobj_t *mo, boolean safe)
{
    ...
    P_SetMobjState(
        mo,
        safe ? P_LatestSafeState(mobjinfo[mo->type].deathstate) : mobjinfo[mo->type].deathstate);
...
void P_ZMovement(mobj_t *mo)
{
    ...
    if (mo->z + mo->height > mo->ceilingz)
    {
        ...
        if ((mo->flags & MF_MISSILE) && !(mo->flags & MF_NOCLIP) && !(mo->flags & MF_BOUNCES))
        {
            P_ExplodeMissile(mo);
            return;
        }
...
void P_RespawnSpecials(void)
{
    ...
    {
        I_Error(
            "P_RespawnSpecials: Failed to find mobj type with doomednum "
...
void P_SpawnMapThing(mapthing_t *mthing)
{
    ...
    if (!(mthing->options & (MTF_EASY | MTF_NORMAL | MTF_HARD)))
    {
        fprintf(
            stderr,
            "P_SpawnMapThing: Mapthing type %i without any skill tag at (%i, %i)\n",
            mthing->type,
            mthing->x,
            mthing->y);
    }

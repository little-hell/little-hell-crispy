...
void P_InitSwitchList(void)
{
    ...
    {
        const short alphSwitchList_episode =
            from_lump ? SHORT(alphSwitchList[i].episode) : alphSwitchList[i].episode;
...
void P_StartButton(line_t *line, bwhere_e w, int texture, int time)
{
    ...
    for (i = 0; i < maxbuttons; i++)
    {
        if (buttonlist[i].btimer && buttonlist[i].line == line)
        {
            ...
            // [crispy] register up to three buttons at once for lines with more than one switch
            // texture
    ...
    for (i = 0; i < maxbuttons; i++)
    {
        if (!buttonlist[i].btimer)
        {
            ...
            buttonlist[i].soundorg =
                crispy->soundfix ? &line->soundorg
                                 : &line->frontsector->soundorg; // [crispy] corrected sound source
            return;
        }
    }

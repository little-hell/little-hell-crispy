...
void HUlib_drawTextLine(hu_textline_t *l, boolean drawcursor)
{
    ...
    for (i = 0; i < l->len; i++)
    {
        ...
        if (c == cr_esc)
        {
            if (l->l[i + 1] >= '0' && l->l[i + 1] <= '0' + CRMAX - 1)
            {
                i++;
                dp_translation =
                    (crispy->coloredhud & COLOREDHUD_TEXT) ? cr[(int) (l->l[i] - '0')] : NULL;
            }
        }
        else
    ...
    if (drawcursor && x + SHORT(l->f['_' - l->sc]->width) <= ORIGWIDTH + WIDESCREENDELTA)
    {
...
void HUlib_eraseTextLine(hu_textline_t *l)
{
    ...
    if (!automapactive && viewwindowx &&
        (l->needsupdate || crispy->cleanscreenshot || crispy->screenshotmsg == 4))
    {
        ...
        lh *= yoffset;
        for (y = (l->y << crispy->hires), yoffset = y * SCREENWIDTH;
             y < (l->y << crispy->hires) + lh;

    ...
    while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0)
    {
        ...
        switch (type)
        {
            case vld_blazeClose:
                ...
                // [crispy] fix door-closing sound playing, even when door is already closed
                // (repeatable walkover trigger)
                if (door->sector->ceilingheight - door->sector->floorheight > 0 ||
                    !crispy->soundfix)
                    S_StartSound(&door->sector->soundorg, sfx_bdcls);
                break;
            ...
            case 117:
                if (door->direction == -1)
                {
                    ...
                    if (crispy->soundfix &&
                        door->thinker.function.acp1 == (actionf_p1) T_VerticalDoor)
                        S_StartSound(
                            &door->sector->soundorg, line->special == 117 ? sfx_bdopn : sfx_doropn);
                }
                else
                {
                    if (!thing->player)
                    ...
                    else
                    {
                        ...
                        fprintf(
                            stderr,
                            "EV_VerticalDoor: Tried to close "

...
unsigned **
    texturecolumnofs; // [crispy] column offsets for composited translucent mid-textures on 2S walls
    ...
    //  if (texture->patchcount > 1 && texture->height < 256)
    {
        ...
        for (i = texture->patchcount, patch = texture->patches; --i >= 0;)
        {
            ...
            for (x = x1; x < x2; x++)
            {
                ...
                {
                    const column_t *col =
                        (const column_t *) ((const byte *) realpatch + LONG(cofs[x]));
        ...
        tranmap = Z_Malloc(256 * 256, PU_STATIC, 0);
        {
            ...
            for (i = 0; i < 256; i++)
            {
                ...
                for (j = 0; j < 256; j++)
                {
                    ...
                    blend[r] =
                        (tran_filter_pct * fg[r] + (100 - tran_filter_pct) * bg[r]) / (100 + btmp);
                    blend[g] =
                        (tran_filter_pct * fg[g] + (100 - tran_filter_pct) * bg[g]) / (100 + btmp);
...
void R_InitColormaps(void)
{
    ...
    if (!colormaps)
    {
        colormaps = (lighttable_t *) Z_Malloc(
            (NUMCOLORMAPS + 1) * 256 * sizeof(lighttable_t), PU_STATIC, 0);
    }
    ...
    if (crispy->truecolor)
    {
        for (c = 0; c < NUMCOLORMAPS; c++)
        {
            ...
            for (i = 0; i < 256; i++)
            {
                r = gamma2table[usegamma][playpal[3 * i + 0]] * (1. - scale) +
                    gamma2table[usegamma][0] * scale;
                g = gamma2table[usegamma][playpal[3 * i + 1]] * (1. - scale) +
                    gamma2table[usegamma][0] * scale;
                b = gamma2table[usegamma][playpal[3 * i + 2]] * (1. - scale) +
                    gamma2table[usegamma][0] * scale;
        ...
        for (i = 0; i < 256; i++)
        {
            const byte gray = 0xff - (byte)(
                                         0.299 * playpal[3 * i + 0] + 0.587 * playpal[3 * i + 1] +
                                         0.114 * playpal[3 * i + 2]);

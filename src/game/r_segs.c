...
void R_RenderMaskedSegRange(drawseg_t *ds, int x1, int x2)
{
    ...
    lightnum =
        (frontsector->rlightlevel >> LIGHTSEGSHIFT) + (extralight * LIGHTBRIGHT); // [crispy] A11Y
    ...
    for (dc_x = x1; dc_x <= x2; dc_x++)
    {
        ...
        if (maskedtexturecol[dc_x] != INT_MAX) // [crispy] 32-bit integer math
        {
            if (!fixedcolormap)
            {
                ...
                dc_colormap[1] =
                    (crispy->brightmaps & BRIGHTMAPS_TEXTURES) ? colormaps : dc_colormap[0];
            }
            ...
            {
                int64_t t =
                    ((int64_t) centeryfrac << FRACBITS) - (int64_t) dc_texturemid * spryscale;
        ...
        else
        {
            ...
            if (toptexture)
            {
                ...
                if (mid >= yl)
                {
                    ...
                    dc_texheight =
                        textureheight[toptexture] >> FRACBITS; // [crispy] Tutti-Frutti fix
            ...
            if (bottomtexture)
            {
                ...
                if (mid <= yh)
                {
                    ...
                    dc_texheight =
                        textureheight[bottomtexture] >> FRACBITS; // [crispy] Tutti-Frutti fix
...
void R_StoreWallRange(int start, int stop)
{
    ...
    if (segtextured)
    {
        ...
        if (!fixedcolormap)
        {
            lightnum = (frontsector->rlightlevel >> LIGHTSEGSHIFT) +
                       (extralight * LIGHTBRIGHT); // [crispy] A11Y

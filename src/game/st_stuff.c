...
void ST_refreshBackground(boolean force)
{
    ...
    if (st_classicstatusbar || force)
    {
        ...
        {
            V_CopyRect(
                ST_X, 0, st_backing_screen, SCREENWIDTH >> crispy->hires, ST_HEIGHT, ST_X, ST_Y);
        }
...
static int ST_cheat_spechits()
{
    ...
    for (i = 0; i < numlines; i++)
    {
        if (lines[i].special)
        {
            ...
            if (lines[i].special == 11 || lines[i].special == 51 || lines[i].special == 52 ||
                lines[i].special == 124 || lines[i].special == 39 || lines[i].special == 97)
            {
                continue;
            }
...
boolean ST_Responder(event_t *ev)
{
    ...
    else if (ev->type == ev_keydown)
    {
        if (!netgame && gameskill != sk_nightmare)
        {
            ...
            for (i = 0; i < 6; i++)
            {
                if (i < 4 ? cht_CheckCheatSP(&cheat_powerup[i], ev->data2)
                          : cht_CheckCheat(&cheat_powerup[i], ev->data2))
                {
                    if (!plyr->powers[i])
                        P_GivePower(plyr, i);
            ...
            // [crispy] implement Boom's "tntem" cheat
                ...
                cht_CheckCheatSP(&cheat_massacre, ev->data2) ||
            ...
            // [crispy] implement PrBoom+'s "notarget" cheat
                ...
                cht_CheckCheatSP(&cheat_notarget, ev->data2) ||
            ...
            else if (cht_CheckCheatSP(&cheat_weapon, ev->data2))
            {
                ...
                // [crispy] TNTWEAP0 takes away all weapons and ammo except for the pistol and 50
                // bullets
                if (w == -1)
                {
                    GiveBackpack(false);
                ...
                else
                {
                    ...
                    {
                        extern boolean P_GiveWeapon(
                            player_t * player, weapontype_t weapon, boolean dropped);
                        ...
                        if (w > 1)
                        {
                            plyr->message = WeaponPickupMessages[w];
                        }
...
#undef BUILD_DATE
        ...
        else if (cht_CheckCheat(&cheat_skill, ev->data2))
        {
            ...
            M_snprintf(
                msg, sizeof(msg), "Skill: %s", skilltable[BETWEEN(0, 5, (int) gameskill + 1)]);
            plyr->message = msg;
        }
        // [crispy] snow
        ...
        if (!netgame && cht_CheckCheat(&cheat_clev, ev->data2) &&
            !menuactive) // [crispy] prevent only half the screen being updated
        {
            char buf[3];
            int epsd;
            int map;
...
void ST_updateWidgets(void)
{
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        if (plyr->cards[i + 3])
            keyboxes[i] = (keyboxes[i] == -1)
                              ? i + 3
                              : i + 6; // [crispy] support combined card and skull keys
...
void ST_doPaletteStuff(void)
{
    ...
    else if (plyr->bonuscount && plyr->health > 0) // [crispy] never show the yellow bonus palette
                                                   // for a dead player
    {
...
static inline boolean ST_PlayerIsGibbed(void)
{
    ...
    return (
        plyr->health <= 0 && ((state >= S_PLAY_XDIE1 && state <= S_PLAY_XDIE9) || state == S_GIBS));
}
...
static inline void ST_DrawGibbedPlayerSprites(void)
{
    ...
    {
        dp_translation =
            translationtables - 256 + ((plyr->mo->flags & MF_TRANSLATION) >> (MF_TRANSSHIFT - 8));
    }
...
void ST_drawWidgets(boolean refresh)
{
    ...
    if (st_crispyhud)
    {
        ...
        if (plyr->readyweapon == wp_fist && plyr->powers[pw_strength])
        {
            ...
            V_DrawPatch(
                ST_AMMOX - 21 - SHORT(patch->width) / 2 + SHORT(patch->leftoffset),
    ...
    if (st_crispyhud && (screenblocks % 3 == 0))
    {
        if (netgame)
            ...
            V_CopyRect(
                ST_FX + WIDESCREENDELTA,
                1,
                st_backing_screen,
                SHORT(faceback[0]->width),
                ST_HEIGHT - 1,
                ST_FX + WIDESCREENDELTA,
                ST_Y + 1);
    }
...
void ST_loadData(void)
{
    ...
    for (i = NUMCARDS; i < NUMCARDS + 3; i++)
    {
        ...
        M_snprintf(lumpname, 9, "STKEYS%d", i);
...
void ST_createWidgets(void)
{
    ...
    // arms background
    STlib_initBinIcon(
        &w_armsbg, ST_ARMSBGX, ST_ARMSBGY, armsbg, &st_notdeathmatch, &st_classicstatusbar);
    ...
    // faces
    STlib_initMultIcon(&w_faces, ST_FACESX, ST_FACESY, faces, &st_faceindex, &st_statusbarface);
...
void ST_Init(void)
{
    ...
    st_backing_screen = (pixel_t *) Z_Malloc(
        MAXWIDTH * (ST_HEIGHT << 1) * sizeof(*st_backing_screen), PU_STATIC, 0);
}

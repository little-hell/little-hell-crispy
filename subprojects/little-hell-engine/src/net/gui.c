...
static void OpenWaitDialog(void)
{
    ...
    TXT_SetWindowPosition(
        window, TXT_HORIZ_CENTER, TXT_VERT_BOTTOM, TXT_SCREEN_W / 2, TXT_SCREEN_H - 9);
...
static void BuildMasterStatusWindow(void)
{
    ...
    TXT_SetWindowPosition(
        master_window, TXT_HORIZ_CENTER, TXT_VERT_CENTER, TXT_SCREEN_W / 2, TXT_SCREEN_H - 4);
...
static void CheckSHA1Sums(void)
{
    ...
    correct_wad =
        memcmp(net_local_wad_sha1sum, net_client_wait_data.wad_sha1sum, sizeof(sha1_digest_t)) == 0;
    correct_deh =
        memcmp(net_local_deh_sha1sum, net_client_wait_data.deh_sha1sum, sizeof(sha1_digest_t)) == 0;
    ...
    else if (!correct_wad)
    {
        TXT_AddWidget(
            window,
            TXT_NewLabel("Your WAD directory does not match other players in the game.\n"
    ...
    if (!correct_deh)
    {
        TXT_AddWidget(
            window,
            TXT_NewLabel("Your dehacked signature does not match other players in the\n"

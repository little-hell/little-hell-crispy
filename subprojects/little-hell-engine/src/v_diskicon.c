...
static void SaveDiskData(const char *disk_lump, int xoffs, int yoffs)
{
    ...
    V_DrawPatch(
        (loading_disk_xoffs >> crispy->hires) - WIDESCREENDELTA,
        loading_disk_yoffs >> crispy->hires,
        disk);
    CopyRegion(
        disk_data,
        LOADING_DISK_W,
        tmpscreen + yoffs * SCREENWIDTH + xoffs,
        SCREENWIDTH,
        LOADING_DISK_W,
        LOADING_DISK_H);
...
void V_EnableLoadingDisk(const char *lump_name, int xoffs, int yoffs)
{
    ...
    saved_background =
        Z_Malloc(LOADING_DISK_W * LOADING_DISK_H * sizeof(*saved_background), PU_STATIC, NULL);
